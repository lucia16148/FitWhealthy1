document.addEventListener("DOMContentLoaded", async function () {
    ...
    if (viewGraphButton && window.location.pathname.includes("miglioramenti.html")) {
        viewGraphButton.addEventListener('click', async function (event) {
        ...
        if (inizio > oggi) {
            await Swal.fire({
                ...
                text: 'La data di inizio non può essere nel futuro.'
            });
            return;
        ...
        if (fine > oggi) {
            await Swal.fire({
                icon: 'error',
                title: 'Data non valida',
                text: 'La data di fine non può essere nel futuro.'
            });
            return;
        ...
        viewGraphButton.addEventListener('click', async function (event) {
            ...
            if (selectedMetrics.length === 0) {
                await Swal.fire({
                    ...
                    text: 'Seleziona almeno una metrica.'
                });
                return;
